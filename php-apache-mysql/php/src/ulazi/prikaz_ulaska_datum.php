                        ...
                        if(mysqli_stmt_execute($stmt))
                        {
                            ...
                            if(mysqli_stmt_num_rows($stmt)>0)
                            {
                                while (mysqli_stmt_fetch($stmt)) {
                                    ...
                                    <tbody>';
                                    $sql = "SELECT * FROM Ulazi WHERE date(Vrijeme) >=? AND date(Vrijeme) <=? ORDER BY ID DESC";
                                    if($stmt=mysqli_prepare($link,$sql))
                                    {
                                        ...
                                        if(mysqli_stmt_execute($stmt))
                                        {
                                            ...
                                            if(mysqli_stmt_num_rows($stmt)>0)
                                            {
                                                while(mysqli_stmt_fetch($stmt))
                                                {
                                                    ...
                                                    if($status=='correct')
                                                    {
                                                        echo '<td class="bg-success">Točna lozinka</td>';
                                                    }
                                                    else if($status=='wrong')
                                                    {
                                                        echo '<td class="bg-warning">Netočna lozinka</td>';
                                                    }
                                                    else if($status=='unknown')
                                                    {
                                                        echo '<td class="bg-warning">Nepoznata kartica</td>';
                                                    }
                                                    else if($status=='block')
                                                    {
                                                        echo '<td class="bg-danger">Blokirana kartica</td>';
                                                    }
                                                    echo "</tr>";
                                                }
                                            }
